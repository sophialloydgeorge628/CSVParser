...
public class CommandLineApplication {
  ...
  public void run() {
    ...
    try (BufferedReader br = new BufferedReader(new InputStreamReader(this.in))) {
      ...
      while ((input = br.readLine()) != null) {
        if (input.equalsIgnoreCase("Please pass in a CSV")) {
          // DEMO FROM CLASS: for debugging not test
          // config file, prompt for password
  ...
  // taking in 2 strings, assuming 1 is a file name; the other is something else

  // they read things in from system input after program starts running
  /**
